...
public final class Checker implements ActualParameterVisitor<FormalParameter, Void>,
	...
	public Void visitSquareCommand(SquareCommand ast, Void unused) {
		// Check the type of the variable being squared
		var vType = ast.V.visit(this);

		// Ensure the variable is of type Integer
		checkAndReportError(vType.equals(StdEnvironment.integerType), "Only integer variables can be squared", ast.V);

		// Ensure the LHS is a variable
		checkAndReportError(ast.V.variable, "LHS of squaring operation is not a variable", ast.V);
