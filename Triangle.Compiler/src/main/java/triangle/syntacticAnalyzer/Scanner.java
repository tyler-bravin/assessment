...
public final class Scanner {
	...
	private Token.Kind scanToken() {
		...
		case '-':
		case '*':
			takeIt(); // Take the first '*'
			if (currentChar == '*') { // Check if the next character is also '*'
				takeIt(); // Take the second '*'
				return Token.Kind.STARSTAR; // Return the new token kind for '**'
			} else {
				return Token.Kind.OPERATOR; // Return the token for a single '*'
			}
		case '/':
		case '=':
		case '<':
